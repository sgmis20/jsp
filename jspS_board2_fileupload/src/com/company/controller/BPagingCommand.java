...
public class BPagingCommand implements BCommand {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		double pstartno = 0;
		if(request.getParameter("pstartno1")!=null) {
			pstartno = Integer.parseInt(request.getParameter("pstartno1"));
		}
		
		//6.최신글부터 10개씩 보여주기
		ArrayList<Board1> dtos = dao.b1list10((int)pstartno, (int)onepagelimit);
		...
		*/
		...
		request.setAttribute("list1p", dtos);
		request.setAttribute("listcount1p", (int)listcount);
		request.setAttribute("onepagelimit1", (int)onepagelimit);
		request.setAttribute("pagetotal1", (int)pagetotal);
		request.setAttribute("bottomlist1", (int)bottomlist);
		request.setAttribute("pstartno1", (int)pstartno);
		request.setAttribute("current_page1", (int)current_page);
		request.setAttribute("start_page1", (int)start_page);
		request.setAttribute("end_page1", (int)end_page);
	}
